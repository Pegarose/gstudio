...
- `npm run test:all` — passed after the final repair-limit change: 32 legacy + 59 unit + 58 integration.
...
- Full production scratch smoke has not yet reached `candidate-ready` followed by apply `complete` after commit `500cca7`. The compact smoke request was deliberately interrupted when the user requested this checkpoint.
...
1. Create a temporary project and E2B sandbox, then send a compact two-file scratch request to `/api/generate-ai-code-stream` using the active OmniRoute role defaults.
2. Assert the generation stream includes `validation` and `candidate-ready` with no `error`; use that candidate in `/api/apply-ai-code-stream` and require terminal `complete` only after live validation passes.
3. Confirm the E2B preview renders the generated application rather than the sandbox-ready page, then kill the sandbox and delete the temporary project.
4. If the quality repair still returns an incomplete artifact, capture its safe validation findings and output finish reason; do not loosen `assertCompleteRepairArtifact`.
