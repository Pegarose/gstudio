...
export function selectSkillsForGeneration({ intent, prompt = '', isEdit = false } = {}) {
  ...
  const explicitArtifactContract = /exactly\s+(?:\d+|one|two|three|four|five|six)\s+files?\b|exact file (?:count|paths?)|only\s+(?:these\s+)?files?\b|no (?:other|additional) files?\b|src\/app\.(?:jsx|tsx)|src\/index\.css/.test(lower);
  const singleComponent = resolvedIntent === 'scratch' && explicitSingleComponent;
  const skills = singleComponent
    ? ['component-scope']
    : explicitArtifactContract
      ? []
      : ['design-core', 'design-intelligence'];
