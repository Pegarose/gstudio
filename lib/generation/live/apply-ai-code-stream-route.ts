...
export interface LiveApplyRouteGeneration {
  id: string;
  projectId: string;
  repairCount: number;
}

export interface LiveApplyRoutePersistence {
  resolveGeneration(context: GenerationContext): Promise<LiveApplyRouteGeneration>;
  prepareActivation(input: {
    generation: LiveApplyRouteGeneration;
    sandboxId: string;
    artifact: GenerationArtifact;
    brief: { contentFacts: string[]; allowedPlaceholders: string[] };
    plan: { primaryCta: null; declaredPackages: string[] };
  }): Promise<void>;
  persistValidation(input: { generationId: string; report: ValidationReport; status: 'passed' | 'failed' }): Promise<void>;
  claimRepairAttempt(generationId: string): Promise<{ id: string; repairCount: number } | null>;
}

export interface LiveApplyActivationFactoryInput {
...
/**
 * This is a deliberately narrow production-route seam. It keeps the exported
 * route's default providers, persistence, sandbox service, and SSE terminal
 * events unchanged while letting integration tests consume the real stream
 * with deterministic collaborators.
 */
export interface ApplyAiCodeStreamRouteDependencies {
  resolveProvider?: (input: { sandboxId?: string }) => Promise<SandboxProvider | undefined>;
  persistence?: LiveApplyRoutePersistence;
  createSandboxService?: (provider: SandboxProvider, sandboxId: string) => ReturnType<typeof createProviderBackedSandboxService>;
  createActivation?: (input: LiveApplyActivationFactoryInput) => ReturnType<typeof createLiveValidationActivation>;
  writeCandidateFile?: typeof writeLiveCandidateFile;
  emitTerminalEvents?: typeof emitLiveActivationTerminalEvents;
}
...
export function createApplyAiCodeStreamRoute(overrides: ApplyAiCodeStreamRouteDependencies = {}) {
  const persistence = overrides.persistence ?? createDefaultLiveApplyRoutePersistence();
  const createSandboxForActivation = overrides.createSandboxService ?? createProviderBackedSandboxService;
  const writeCandidate = overrides.writeCandidateFile ?? writeLiveCandidateFile;
  const emitTerminalEvents = overrides.emitTerminalEvents ?? emitLiveActivationTerminalEvents;
  ...
  try {
    ...
    try {
      artifact = toValidationArtifact(parsed);
    } catch (error) {
      if (error instanceof UnsafeGeneratedCandidatePathError) {
        // Invalid candidate paths are a request-contract failure, not a live
        // activation failure. Returning 400 here guarantees no sandbox,
        // persistence, repair, or mutation-barrier side effect can occur.
        return NextResponse.json({
          error: 'Unsafe generated file path in candidate.',
        }, { status: 400 });
      }
      throw error;
    }
    ...
    if (!provider) {
      return NextResponse.json({
        error: 'No sandbox provider is available for this apply request.',
      }, { status: 500 });
    }
    ...
    const generation = await persistence.resolveGeneration(generationContext);
    const brief = { contentFacts: [], allowedPlaceholders: [] };
    const plan = { primaryCta: null, declaredPackages: artifact.packages ?? [] };
    await persistence.prepareActivation({
      generation,
    ...
    (async (providerInstance, req) => {
      ...
      try {
        ...
        const sandboxService = createSandboxForActivation(providerInstance as SandboxProvider, liveSandboxId);
        const activation = overrides.createActivation?.({
        ...
        // can emit its terminal error instead of waiting on an unreadable SSE.
...
class UnsafeGeneratedCandidatePathError extends Error {
  constructor() {
    super('Unsafe generated candidate path.');
    this.name = 'UnsafeGeneratedCandidatePathError';
  }
}

function normalizeGeneratedCandidateFiles(files: ParsedResponse['files']): ParsedResponse['files'] {
  return files.map((file) => {
    const normalizedPath = posixPath.normalize(file.path);
    if (
      file.path.split('/').includes('..') ||
      !SAFE_GENERATED_CANDIDATE_PATH.test(normalizedPath)
    ) {
      throw new UnsafeGeneratedCandidatePathError();
    }
...
function createDefaultLiveApplyRoutePersistence(): LiveApplyRoutePersistence {
  return {
    resolveGeneration,
    prepareActivation: async ({ generation, sandboxId, artifact, brief, plan }) => {
      await Promise.all([
        setGenerationSandboxId(generation.id, sandboxId),
...
function createDefaultLiveValidationActivation(input: Pick<LiveApplyActivationFactoryInput, 'provider' | 'sandbox' | 'persistence'>) {
  ...
  return createLiveValidationActivation({ sandbox: input.sandbox, orchestrator });
}
